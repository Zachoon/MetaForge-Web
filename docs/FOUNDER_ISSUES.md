...
*Last updated: 2026-08-19 — #030 shipped: archetype-catalog batch 3 adds 6 more declarative packages (legends, discard, graveyard, clones, flying, group slug) on top of #028's proof of concept and #029's batch 2 — zero new false-friend shapes; all 6 reuse `broad-type-superset`, `incidental-rider`, or `wrong-target-scope`, with `wrong-target-scope` reused across three further, structurally distinct sub-domains (object-type scope for clones, keyword-vs-payoff scope for flying, trigger-subject scope for group_slug), reconfirming it as a genuinely general shape rather than a player-scope-only tool. 104-forge field: pass 100%, hard failures 0, no hard regressions against the frozen baseline. All 16 pre-existing archetype/package occupancy tests (76 sub-tests) plus `validate:founder-026`/`validate:founder-027` re-verified green with zero behavior change to any existing entry. `npm run lint`/`npm run build` both pass with zero findings in any changed file.*
