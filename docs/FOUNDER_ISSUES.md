...
### #035 — Test-coverage gap closure: auras, equipment, blink occupancy

| | |
|---|---|
| **Layer** | Strategic / Construction (tests only) |
| **Brain impact** | None — zero behavior/logic change anywhere. |

**Scope**

Of the 10 original hand-authored `PACKAGE_CATALOG` entries in `strategic-intent.mjs`, 7 (`aristocrats`, `reanimator`, `tokens`, `landfall`, `typal`, `spellslinger`, `stax`) already had a dedicated `tests/*-package-occupancy.test.mjs` file. `auras`, `equipment`, and `blink` never did — a gap flagged since #028's own scope note ("the exact generic-dispatch path `auras`/`equipment`/`blink` already run through") but never closed, even as the same rigor was applied to all 38 archetype-catalog entries across #028–#034. This issue closes it: pure test-writing, zero changes to `auras`, `equipment`, `blink`, or any other production code.

**Legal next step (done 2026-08-19)**

Three new files: `tests/auras-package-occupancy.test.mjs`, `tests/equipment-package-occupancy.test.mjs`, `tests/blink-package-occupancy.test.mjs`, matching the existing 7's convention (`intentFor` helper, real Scryfall-verified card/commander fixtures, one live `forgeNativeMasterwork` construction test each). New `package.json` scripts `validate:auras-occupancy`, `validate:equipment-occupancy`, `validate:blink-occupancy`.

Real commanders used: Eriette of the Charmed Apple (auras), Kemba, Kha Regent (equipment), Emiel the Blessed (blink). Real core/false-friend/support fixtures: Pacifism vs. Bitterblossom vs. Swiftfoot Boots/Hateful Eidolon (auras); Umezawa's Jitte vs. Sol Ring vs. Spirit Mantle (equipment); Ephemerate vs. Karmic Guide vs. Mulldrifter/Restoration Angel (blink).

**Findings surfaced while writing (documented, not fixed — out of scope per this issue's own brief)**

Writing real-card evidence against these 3 packages' unmodified logic surfaced four genuine discrepancies, each now pinned by an explicit, passing test asserting the actual (current) behavior:

1. **Uril, the Miststalker — the format's most iconic Aura-Voltron commander — never opens the `auras` package.** Its current oracle text ("Hexproof. Uril gets +2/+2 for each Aura attached to it.") is a static ability with no "whenever", no "auras you control", no "enchanted creature" phrasing, so it trips none of `detectAurasCommander`'s four alternatives.
2. **Roon of the Hidden Realm and Restoration Angel — two of the format's most-played Blink cards — never trip `blink_effect`/`detectBlinkCommander`.** The regex requires the literal sequence `return it/that/them to the battlefield`; real Wizards templating for the majority of blink cards (Roon, Brago, Thassa Deep-Dwelling, Cloudshift, Flickerwisp, Displace, Felidar Guardian, Restoration Angel) reads "return **that card**/those cards to the battlefield" — one extra word that never matches. Only the minority phrased exactly "return it to the battlefield" (Ephemerate, Momentary Blink, Emiel the Blessed, Aminatou) actually qualify as core or trigger commander detection.
3. **`blink`'s `falseFriendSemantics` is an empty array — there is no false-friend protection for this package at all**, unlike `auras`/`equipment`'s populated `non_aura_enchantment`/`non_equipment_artifact` pairs. A pure graveyard-reanimation card (Karmic Guide: "return target creature card from your graveyard to the battlefield") is correctly excluded from core but is never flagged as a false friend either — `cardIsPackageFalseFriend` just falls through an empty list and returns false.
4. **`blink`'s `supportSemantics: ["etb_value"]` is generic enough to credit that same reanimation card as real support.** `cardSatisfiesPackageSupport(karmicGuide, "blink", intent)` returns `true`, because `etb_value`'s predicate list (`draw|create|exile target|gain|put|return|search`) includes bare "return" with no requirement that anything was exiled first — so a reanimation spell counts toward blink density.
5. **A card can simultaneously trip a support tag and a false-friend tag for `auras`.** Hateful Eidolon carries both `aura_payoff` (support) and `non_aura_enchantment` (false friend); `cardIsPackageFalseFriend` never consults `supportSemantics`, so the two functions disagree with each other about the same card.

None of these are fixed here — this issue is scoped to test coverage only, and the task that opened it was explicit that discrepancies get reported, not patched. #1 and #2 look like the more consequential ones (real, popular commanders silently failing to open their own package); #3/#4/#5 are narrower mechanism gaps specific to `blink`/`auras`. All are left for a future, dedicated Founder Issue.

Tests: `npm run validate:auras-occupancy` (6 sub-tests), `validate:equipment-occupancy` (5 sub-tests), `validate:blink-occupancy` (6 sub-tests) — 17 new sub-tests total, each with commander-path, core/false-friend/support unit coverage, and one live `forgeNativeMasterwork` construction test against a real, Scryfall-verified commander. All 45 pre-existing `*-package-occupancy` test files (255 sub-tests) plus `validate:founder-026` (26 sub-tests) and `validate:founder-027` (9 sub-tests) re-verified green with zero behavior change — 290 sub-tests total across all 48 occupancy files (confirmed via `ls tests/*-package-occupancy.test.mjs | wc -l` = 48) plus both founder suites, all passing after this issue's 17 new sub-tests. `npm run lint` and `npm run build` both pass — no lint findings in any new or touched file (`app/strategic-intent.mjs` has two pre-existing unused-var warnings, both untouched by this issue, confirmed via `git diff --stat` showing zero lines changed in that file).

**Live trial:** N/A — test coverage only, no Brain-facing behavior changed.
...
*Last updated: 2026-08-19 — #035 shipped: closes the test-coverage gap #028 flagged and never closed — `auras`, `equipment`, and `blink`, the 3 of the original 10 hand-authored `PACKAGE_CATALOG` entries that never got a dedicated `*-package-occupancy` test file even after all 38 archetype-catalog entries did. Pure test-writing, zero behavior/logic change to `auras`, `equipment`, `blink`, or any other production file (confirmed via `git diff --stat`, zero lines changed in `app/strategic-intent.mjs`). Adds `tests/auras-package-occupancy.test.mjs`, `tests/equipment-package-occupancy.test.mjs`, `tests/blink-package-occupancy.test.mjs` (17 new sub-tests, real Scryfall-verified commanders Eriette of the Charmed Apple, Kemba Kha Regent, Emiel the Blessed) plus matching `validate:*-occupancy` npm scripts. Writing real-card evidence against these 3 packages' unmodified logic surfaced 5 genuine discrepancies, each pinned by a passing test documenting current behavior and reported (not fixed) per this issue's own scope: Uril, the Miststalker never opens `auras`; Roon of the Hidden Realm and Restoration Angel never trip `blink_effect`/`detectBlinkCommander` (real "return that card" templating vs. the regex's literal "return it"); `blink`'s `falseFriendSemantics` is empty (no false-friend protection exists for it at all); `blink`'s `etb_value` support tag is broad enough to credit a pure reanimation card (Karmic Guide) as blink support; and a card can simultaneously trip a support tag and a false-friend tag for `auras` since `cardIsPackageFalseFriend` never consults `supportSemantics` (Hateful Eidolon). All 45 pre-existing `*-package-occupancy` test files (255 sub-tests) plus both founder suites (35 sub-tests) re-verified green with zero behavior change — 290 sub-tests total across all 48 occupancy files (confirmed via `ls tests/*-package-occupancy.test.mjs | wc -l` = 48) plus both founder suites, all passing after this issue's 17 new sub-tests. `npm run lint`/`npm run build` both pass with zero findings in any new or touched file. Not a Brain-facing change — no Live Founder Trial applicable.*
