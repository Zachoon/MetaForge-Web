...
- Commander guide pages (`/commanders`, `/commanders/[slug]`) are a second text-first discovery surface, launched 2026-08-19 with a curated batch of 5 heavily-played commanders (see `app/commanders/data.mjs`). Content is derived entirely from the commander's real oracle text via the same occupancy-engine analysis used in the live product — never fabricated or copied from EDHREC-style popularity data. Expanding the batch is a scoping decision (prove a small batch performs before growing it), not a code change — add entries to `COMMANDER_GUIDES` in `app/commanders/data.mjs` and mirror the new URLs into `SITEMAP_URLS` in `worker/index.ts`. The "Build this deck" CTA uses a public `?commander=<slug>` key (never internal engine state), resolved client-side in `app/page.tsx`, matching the Academy `?guide=` pattern.
