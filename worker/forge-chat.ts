...
export async function handleForgeChat(request: Request, env: ChatEnv) {
  ...
  const upstream = await fetch("https://api.openai.com/v1/responses", { method:"POST", headers:{ Authorization:`Bearer ${env.OPENAI_API_KEY}`, "Content-Type":"application/json" }, body:JSON.stringify({ model:env.OPENAI_MODEL || "gpt-5.2", input:[{role:"system",content:SYSTEM},{role:"system",content:context},...messages], reasoning:{effort:"low"}, max_output_tokens:1100, store:false }) });
  ...
  return answer ? json({ answer, model:env.OPENAI_MODEL || "gpt-5.2", remaining:founder ? null : 10-used, resetAt:reset.toISOString(), evidenceBoundary:"Personalized guidance grounded in game-separated verified context and approved coaching knowledge; not automatic global training." }) : json({error:"The Forge returned no readable answer"},502);
}
