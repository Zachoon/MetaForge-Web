...
const GAMES=new Set(["mtg","riftbound"]);
...
export async function handleCoachingKnowledge(request:Request,env:Env,founderOnly=false){
  ...
  if(request.method==="GET"){
    ...
    if(!founderOnly){const url=new URL(request.url),format=url.searchParams.get("format"),game=url.searchParams.get("game")||"mtg";claims=claims.filter((c:any)=>c.game===game);if(format)claims=claims.filter((c:any)=>c.format==="General"||c.format===format)}
    return json({claims});
  }
  ...
  if(request.method==="POST"){
    const game=String(body.game||"mtg").trim().toLowerCase(),sourceUrl=String(body.sourceUrl||"").trim(),sourceTitle=String(body.sourceTitle||"").trim(),author=String(body.author||"").trim(),summary=String(body.summary||"").trim(),principle=String(body.principle||"").trim(),publishedAt=String(body.publishedAt||"").trim(),sourceType=String(body.sourceType||"").trim(),stance=String(body.stance||"contextual").trim(),format=String(body.format||"General").trim();
    ...
    if(!GAMES.has(game)||!sourceTitle||!author||summary.length<20||principle.length<8||!/^\d{4}-\d{2}-\d{2}$/.test(publishedAt)||!SOURCE_TYPES.has(sourceType)||!STANCES.has(stance))return json({error:"Complete provenance, game identity, and strategic claim required"},400);
    const id=await idFor(`${game}|${sourceUrl.toLowerCase()}|${author.toLowerCase()}|${principle.toLowerCase()}|${stance}`);
    await env.DB.prepare("INSERT OR IGNORE INTO coaching_knowledge_claims (id,game,source_url,source_title,author,published_at,source_type,summary,principle,format,stance,tags_json,cards_json,status) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?, 'quarantined')").bind(id,game,sourceUrl,sourceTitle,author,publishedAt,sourceType,summary,principle,format,stance,JSON.stringify(cleanList(body.tags)),JSON.stringify(cleanList(body.cards))).run();
    return json({id,game,status:"quarantined"},201);
  }
