...
type StoredReport = { slug: string; title: string; commander_name: string; format_name: string; strategy_name: string; summary: string; deck_rows_json: string; created_at: string; updated_at: string; view_count?: number; share_count?: number };
...
async function loadPublicReport(env: PublicReportEnv, slug: string) {
  return env.DB.prepare("SELECT slug,title,commander_name,format_name,strategy_name,summary,deck_rows_json,created_at,updated_at FROM public_deck_reports WHERE slug=?").bind(slug).first<StoredReport>();
}
