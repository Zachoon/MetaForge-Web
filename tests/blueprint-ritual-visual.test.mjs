...
// The commission/refine chamber's JSX (build stepper, commander search and
// its body portal, commission-note, awaken-button) moved to its own
...
test("the Blueprint chamber presents choices as a completed commissioning ritual", () => {
  ...
  assert.match(commissionChamber, /className=\{`commission-scroll[^`]*`\}|className="commission-scroll"/);
  assert.match(commissionChamber, /className="mark-grid"/);
  assert.match(commissionChamber, /className="commission-note"/);
  assert.match(commissionChamber, /className="awaken-button"/);
...
test("commander choice becomes a responsive, motion-safe Blueprint seal", () => {
  ...
  assert.match(commissionChamber, /className="commander-blueprint(?: [^"]*)?"/);
...
test("commander predictions dismiss instead of leaving an empty listbox", () => {
  assert.match(commissionChamber, /commanderSearchOpen/);
  assert.match(commissionChamber, /event\.key === "Escape"/);
  assert.match(commissionChamber, /event\.currentTarget\.contains\(related\)/);
  assert.match(commissionChamber, /commanderSearchOpen && \(commanderSearching/);
  assert.match(commissionChamber, /setCommanderSearchOpen\(false\)/);
});
...
test("the commander search dropdowns escape .commission-chamber's stacking context via a body portal", () => {
  ...
  assert.match(commissionChamber, /import \{ createPortal \} from "react-dom";/);
  assert.match(commissionChamber, /createPortal\(\s*<div\s*\n\s*role="listbox"\s*\n\s*className="commander-search-portal"/);
  assert.match(commissionChamber, /document\.body,\s*\)/);
  ...
  const portalCount = (commissionChamber.match(/createPortal\(/g) || []).length;
  ...
  assert.match(commissionChamber, /related\.closest\(".commander-search-portal"\)/);
});
...
test("the search dropdown also isn't clipped by .commission-chamber, the ancestor a level up", () => {
  ...
  assert.match(commissionChamber, /className="commission-chamber-sweep"/);
});
