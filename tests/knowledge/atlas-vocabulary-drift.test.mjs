...
describe("Atlas Vocabulary Registry — drift protection", () => {
  ...
  it("never assigns the same seat id or capability id to two different seats", () => {
    ...
    const seatRows = [
      ...
      ...registry.staxSeats,
      ...registry.aurasOccupancySeats,
      ...registry.equipmentOccupancySeats,
      ...registry.blinkSeats,
      ...registry.tokensOccupancySeats,
      ...registry.packageHealthSeats,
  ...
  it("never admits a Capability anywhere in the registry (Coverage Observation 001 stands)", () => {
    ...
    const allCapabilities = [
      ...
      ...registry.aurasOccupancySeats.map((row) => row.capability),
      ...registry.equipmentOccupancySeats.map((row) => row.capability),
      ...registry.blinkSeats.map((row) => row.capability),
      ...registry.tokensOccupancySeats.map((row) => row.capability),
      ...registry.packageHealthSeats.map((row) => row.capability),
