...
export default function Home() {
  ...
  const [coachDepth, setCoachDepth] = useState<"quick"|"balanced"|"deep">("balanced");
  ...
  useEffect(() => { setCoachingProfile(window.localStorage.getItem("metaforge.coachingProfile") || ""); const savedDepth=window.localStorage.getItem("metaforge.coachDepth");if(savedDepth==="quick"||savedDepth==="deep"||savedDepth==="balanced")setCoachDepth(savedDepth); setDebriefHistory(JSON.parse(window.localStorage.getItem("metaforge.debriefs") || "[]")); }, []);
  ...
  async function sendForgeMessage(contentOverride?: string) {
    ...
    try {
      const response = await fetch("/api/forge/chat", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ messages:next, depth:coachDepth, context:{ game:"mtg", deckName, format, deckText, coachingProfile } }) });
    ...
    <main className={`game-shell mtg-mode ${analysisActivity==="analyzing"||arenaStatus==="connecting"||forgeChatStatus==="thinking"?"forge-awake":""} ${forgeStirring?"forge-stirring":""}`}>
            ...
            <div className={`result-panel ${analyzed ? "ready" : ""}`} aria-live="polite">
              {!analyzed ? (
                ...
                <div className="analysis-result">
                  ...
                  <article className="finding"><small>FORGE PICK · {recommendation.evidence?.label || "CONTROLLED HYPOTHESIS"}</small><h4>{recommendation.title}</h4><p>{recommendation.summary}</p><em className="evidence-basis">Basis: {recommendation.evidence?.basis || "deck composition"}</em><details className="why-drawer"><summary>Why this change?</summary><p>{recommendation.reasoning}</p>{recommendation.guardrail && <p><b>Keep working:</b> {recommendation.guardrail}</p>}</details></article>
      ...
      <div className={`forge-chat-dock ${forgeChatOpen ? "open" : ""} ${coachUndocked ? "undocked" : "docked"}`} style={forgeChatOpen && coachUndocked ? { left: coachPosition.x, top: coachPosition.y } : undefined}>
        {!forgeChatOpen ? <button disabled={coachReady!==true} title={coachReady===false?"Coach temporarily offline; deck analysis still works.":"Talk to the Forge"} onClick={() => setForgeChatOpen(true)}><span>✦</span> {coachReady===false?"Coach temporarily offline":"Talk to the Forge"}</button> : <section aria-label="Talk to the Forge">
          ...
          <div className="forge-prompts"><button onClick={() => setForgeChatInput("Build me a fun deck around this idea, then explain the weak points.")}>Build an idea</button><button onClick={() => setForgeChatInput("Explain this recommendation and its runner-up in more depth.")}>Explain the why</button><button onClick={() => setForgeChatInput("Restate your last answer more simply and give me only the decision and next test.")}>Simplify that</button><button onClick={() => setForgeChatInput("Go deeper on your last answer. Show the assumptions, strongest alternative, and what evidence would change your conclusion.")}>Go deeper</button><button onClick={() => setForgeChatInput("Challenge your last recommendation. Make the strongest case against it, then tell me whether you still stand by it.")}>Challenge it</button></div>
          <footer><textarea disabled={coachReady!==true||forgeChatStatus === "thinking"} value={forgeChatInput} onChange={(event) => setForgeChatInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); sendForgeMessage(); } }} placeholder={coachReady===false?"Coach is temporarily offline. Forge deck analysis remains available.":forgeChatStatus === "thinking"?"The Forge is working on your answer…":"Ask about a deck, pick, matchup, mechanic, or wild theory…"} /><button disabled={coachReady!==true||!forgeChatInput.trim() || forgeChatStatus === "thinking" || forgeQuestionsRemaining === 0} onClick={() => sendForgeMessage()}>{forgeChatStatus === "thinking" ? "Forging your answer…" : "Forge answer →"}</button><small>{coachReady===false?"Coach is offline while its secure model connection is configured. No question was charged.":forgeQuestionsRemaining === null ? "Founder unlimited · buddies receive 10 questions weekly" : `${forgeQuestionsRemaining} of 10 questions remaining${forgeQuestionsReset ? ` · resets ${new Date(forgeQuestionsReset).toLocaleDateString()}` : ""}`} · not automatic global training</small></footer>
        </section>}
      </div>
