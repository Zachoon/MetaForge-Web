...
export default function Home() {
  ...
  useEffect(() => { if (chamber !== "workbench" || cardSearch.trim().length < 2) { setCardSearchResults([]); return; } const timer = window.setTimeout(async () => { try { const identity = selectedCommander?.colors?.length ? ` id<=${selectedCommander.colors.join("").toLowerCase()}` : "", arena = format === "Brawl" ? " game:arena" : ""; const query = encodeURIComponent(`legal:${format.toLowerCase()}${arena}${identity} name:${cardSearch.trim()}`); const response = await fetch(`https://api.scryfall.com/cards/search?q=${query}&order=edhrec`); const data = await response.json(); setCardSearchResults((data.data || []).slice(0, 8).map((card: any) => ({ name: card.name, typeLine: card.type_line || "Card", image: card.image_uris?.small || card.card_faces?.[0]?.image_uris?.small || "" }))); } catch { setCardSearchResults([]); } }, 280); return () => window.clearTimeout(timer); }, [cardSearch, chamber, format, selectedCommander?.name]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => { setCommanderSearching(true); try { const query = encodeURIComponent(`${format === "Brawl" ? "game:arena " : ""}legal:${format.toLowerCase()} is:commander name:${commanderQuery.trim()}`); const response = await fetch(`https://api.scryfall.com/cards/search?q=${query}&order=name`); const data = await response.json(); setCommanderResults((data.data || []).slice(0, 8).map(commanderOption)); } catch { setCommanderResults([]); } finally { setCommanderSearching(false); } }, 320);
    return () => window.clearTimeout(timer);
